...
const RenderTableAttributes = ({ propertiesToCompare }) => (
  <Attributes>
    <p className="odd first">Producto</p>
    {propertiesToCompare &&
      propertiesToCompare?.length &&
      propertiesToCompare?.map(({ propertyname }, index) => (
        <p key={`productData-${index}`} className={isOdd(index) ? 'odd' : ''}>
...
const RenderProductData = ({ properties }) => {
  const renderData = data => {
    const text =
      data?.toUpperCase() !== 'SI' && data.toUpperCase() !== 'NO'
        ? data
        : data?.toUpperCase() === 'SI'
        ? 'SI'
        : 'NO'
...
const RenderProductInformation = ({
  ...
  return (
    productsToCompare &&
    productsToCompare.map(({ name, image, link, featurestocompare }, key) => (
      <ProductInformation
        key={`product-${key}`}
        width={`${100 / productsToCompare.length}%`}
      >
        <div
          className={`product-image-wrapper ${key === 0 ? 'first' : ''} ${
            key === productsToCompare && productsToCompare.length - 1
              ? 'last'
              : ''
          }`}
        >
      ...
      <div className="block">
        ...
        <Comparation>
          ...
          <div className="product-information">
            {productsToCompare && productsToCompare?.length && (
